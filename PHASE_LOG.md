...
- Utilities, all tested:
  ...
  - `utils/io.py` — atomic JSON / JSONL / Parquet via same-directory temp + `os.replace`.
...
- `CLAUDE.md` with all 8 invariants verbatim plus a table of how each is enforced
  mechanically; `DECISIONS.md` with D-001…D-006; `README.md`; Apache-2.0 `LICENSE`;
...
| transformers / peft / bitsandbytes / accelerate / trl | 4.44.2 / 0.12.0 / 0.43.3 / 0.33.0 / 0.9.6 |
| vllm | 0.6.2 (independently pins torch 2.4.0 — see D-005) |

PyG companion wheels come from `https://data.pyg.org/whl/torch-2.4.0+cu121.html`.
